...
if not df.empty:
    ...
    st.info("💡 Note: Database file (`real_estate.db`) is not uploaded to GitHub. Predictor remains operational.")
...
if st.sidebar.button("Predict Estimated Price"):
    if model is not None:
        ...
        features = pd.DataFrame([[surface_input, rooms_input, lat, lon]], 
                                columns=['surface_m2', 'rooms', 'latitude', 'longitude'])
        pred = model.predict(features)[0]
        st.sidebar.success(f"Estimated Price: **{pred:,.2f} TND**")
    else:
        st.sidebar.error("Model file not found! Please check `models/real_estate_model.pkl` on GitHub.")
